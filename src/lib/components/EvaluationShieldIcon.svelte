<script lang="ts">
	...
	export let sectionTier: Record<string, number>;
</script>
...
<div class="relative">
	<svg class="w-full h-full" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
		...
		<path
			...
			fill="none"
			stroke={sectionTier.numerator === sectionTier.denominator ? 'black' : 'maroon'}
			stroke-width="1"
		/>
	</svg>
	...
	<p
		class={`absolute m-0 text-center text-lg top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 ${
			sectionTier.numerator === sectionTier.denominator ? '' : 'text-red-800'
		}`}
	>
		{`${sectionTier.numerator}/${sectionTier.denominator}`}
	</p>
</div>
